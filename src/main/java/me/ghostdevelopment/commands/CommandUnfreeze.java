...
public class CommandUnfreeze implements CommandExecutor {
    ...
    VanteyFreeze plugin;
    public CommandUnfreeze(VanteyFreeze plugin){this.plugin=plugin;}
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Utils.Color(plugin.getConfig().getString("unfreeze.usage")));
            return false;
        }
        ...
        if(target==null){
            sender.sendMessage(Utils.Color(plugin.getConfig().getString("unfreeze.player-not-found")).replace("%player%", target.getName()));
            return false;
        }
        ...
        if(args.length == 1){
            ...
            if(target.hasPermission("vanteyfreeze.staff")){
                ...
                if(target == sender){
                    sender.sendMessage(Utils.Color(plugin.getConfig().getString("unfreeze.no-self")));
                    return false;
                }
                if(!(Manager.freezeManager.contains(target))){
                    sender.sendMessage(Utils.Color(plugin.getConfig().getString("unfreeze.already-unfrozen").replace("%player%", target.getName())));
                }else {
                    Manager.freezeManager.remove(target);
                    sender.sendMessage(Utils.Color(plugin.getConfig().getString("unfreeze.player-unfreeze").replace("%player%", target.getName())));
                }
            }
